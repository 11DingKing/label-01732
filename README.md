...
一套面向游戏运营场景的卡密全生命周期管理系统，基于 Spring Boot 2.7 + MyBatis-Plus + MySQL 构建后端，Vue 3 + Element Plus + Pinia 构建管理后台前端，支持 Docker Compose 一键部署。系统涵盖批量发卡（一次可生成 10000 张 9 位唯一卡号 + 6 位随机密码）、按批次或单张回收、卡密核销与核销记录查询、Excel 导出、公开卡密状态查询（无需登录）等核心业务，同时提供 JWT 认证、管理员/操作员角色权限控制、BCrypt 密码加密、操作日志审计、XSS 防护、公开接口频率限制等安全与运维能力。
